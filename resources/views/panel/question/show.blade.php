                ...
                <div id="questions-container-{{ $section->id }}" class="question">
                    @foreach($questions->where('section_id', $section->id) as $qIndex => $q)
                        ...
                        <div class="question" id="question_{{ $section->id }}_{{ $qIndex }}" style="display: {{ $qIndex == 0 ? 'block' : 'none' }}">
                            ...
                            @if($q->type == 'text')
                                <input type="text" name="answers[{{ $section->id }}][{{ $qIndex }}]" class="form-control" placeholder="Enter your answer">
                            @elseif($q->type == 'choice')
                                <div>
                                    @foreach(json_decode($q->options) as $option)
                                        <label>
                                            <input type="radio" name="answers[{{ $section->id }}][{{ $qIndex }}]" value="{{ $option }}">
                                            {{ $option }}
                                        </label><br>
                                    @endforeach
                                </div>
                            @elseif($q->type == 'checkbox')
                                <div>
                                    @foreach(json_decode($q->options) as $option)
                                        <label>
                                            <input type="checkbox" name="answers[{{ $section->id }}][{{ $qIndex }}][]" value="{{ $option }}">
                                            {{ $option }}
                                        </label><br>
                                    @endforeach
                                </div>
                            @elseif($q->type == 'date')
                                <input type="date" name="answers[{{ $section->id }}][{{ $qIndex }}]" class="form-control">
                                @elseif($q->type == 'file')
                            ...
                            <div id="rating_{{ $q->id }}" class="rating" data-question-id="{{ $q->id }}">
                                <input type="hidden" name="answers[{{ $section->id }}][{{ $qIndex }}]" id="selectedRating_{{ $q->id }}" value="0">
                                <span class="star" data-index="1" onclick="setRating({{ $q->id }}, 1)" onmouseover="highlightStars({{ $q->id }}, 1)" onmouseout="resetStars({{ $q->id }})" style="font-size: 30px; cursor: pointer;">&#9733;</span>
                                <span class="star" data-index="2" onclick="setRating({{ $q->id }}, 2)" onmouseover="highlightStars({{ $q->id }}, 2)" onmouseout="resetStars({{ $q->id }})" style="font-size: 30px; cursor: pointer;">&#9733;</span>
                                <span class="star" data-index="3" onclick="setRating({{ $q->id }}, 3)" onmouseover="highlightStars({{ $q->id }}, 3)" onmouseout="resetStars({{ $q->id }})" style="font-size: 30px; cursor: pointer;">&#9733;</span>
                                <span class="star" data-index="4" onclick="setRating({{ $q->id }}, 4)" onmouseover="highlightStars({{ $q->id }}, 4)" onmouseout="resetStars({{ $q->id }})" style="font-size: 30px; cursor: pointer;">&#9733;</span>
                                <span class="star" data-index="5" onclick="setRating({{ $q->id }}, 5)" onmouseover="highlightStars({{ $q->id }}, 5)" onmouseout="resetStars({{ $q->id }})" style="font-size: 30px; cursor: pointer;">&#9733;</span>
                            </div>


                            @elseif($q->type == 'textarea')
                                <textarea name="answers[{{ $section->id }}][{{ $qIndex }}]" class="form-control" rows="4" placeholder="Enter your answer"></textarea>
                            @endif
                        </div>
                    @endforeach
                </div>
