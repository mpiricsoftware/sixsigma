...
$(function () {
    ...
    var dt_user_table = $('.datatables-admin'),
        select2 = $('.select2'),
        ...
        var dt_user = dt_user_table.DataTable({
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    orderable: false,
                    render: function (data, type, full, meta) {
                        return `<span>${full.subplan_id}</span>`;
                    }
                },
                {
                    // Actions
            ...
            buttons: [
                ...
                {
                    text: '<i class="ri-add-line ri-16px me-0 me-sm-2 align-baseline"></i><span class="d-none d-sm-inline-block">Add New Admin</span>',
    ...
    $(document).on('click', '.edit-record', function () {
        ...
        $.get(`${baseUrl}user-list\/${user_id}\/edit`, function (data) {
            ...
            $('#subplan_id').val(data.subplan_id);
        });
    });
...
$('.subgroup').on('change', function()
{
    ...
    var currentModal = $(this).closest('.offcanvas'); // Get the current modal
    var subplan_select = currentModal.find('.subplan_id');
    $.ajax({
        url: 'admin/add/subplans',
        method: "GET",
        data: { subgroup_id: subgroupid },
        dataType: 'json',
        success: function(response) {

            subplan_select.empty();
            subplan_select.append('<option value="">Select Plan</option>');
            $.each(response.subplan, function(index, subplan) {
              subplan_select.append('<option value="' + subplan.id + '">' + subplan.name + '</option>');
            });
        },
